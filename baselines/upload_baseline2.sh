...
# All overridable from the environment. The defaults are placeholders: the host is
# long gone, and AUDIO_GRAFT_ROOT is a different repository that cannot be derived
# from this one's location.
LAMBDA_IP="${LAMBDA_IP:-YOUR_HOST_OR_IP}"
LAMBDA_USER="${LAMBDA_USER:-ubuntu}"
LAMBDA_DIR="${LAMBDA_DIR:-~/audiograft/SurgViVQA-Audio/baselines}"
...
scp -r ${AUDIO_GRAFT_ROOT}/transformers_fork/src/transformers/models/qwen2_vl \

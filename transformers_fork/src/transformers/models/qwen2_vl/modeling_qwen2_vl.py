...
from ...models.whisper.modeling_whisper import WhisperModel
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # IMPORTANT: This must come AFTER post_init() to preserve pre-trained weights
        print("🎧 Grafting Audio Encoder: openai/whisper-large-v3-turbo...")
        # Load Whisper without dtype specification to get pretrained weights in native format
        whisper_backbone = WhisperModel.from_pretrained("openai/whisper-large-v3-turbo")
        ...
        # Convert audio encoder to match main model dtype (good practice for float32/float16/bfloat16)
        target_dtype = next(self.model.parameters()).dtype
        self.audio_encoder = self.audio_encoder.to(dtype=target_dtype)

        # Projection layer: Whisper hidden_size (1280) -> Qwen hidden_size
    ...
    def forward(
        ...
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        input_features: Optional[torch.FloatTensor] = None,  # Audio: Whisper spectrograms
        ...
        if inputs_embeds is None:
            ...
            # Audio Grafting: Process audio features
            if input_features is not None and hasattr(self, 'audio_encoder'):
                ...
                # Count audio tokens and features
                ...
                # Create mask and scatter audio embeddings (same pattern as video)
                audio_mask = (
    ...
    def prepare_inputs_for_generation(
        ...
        image_grid_thw=None,
        video_grid_thw=None,
        input_features=None,  # ADDED THIS LINE
        **kwargs,
    ):
        ...
        model_inputs.update(
            {
                ...
                "input_features": input_features,  # ADDED THIS LINE
                "cache_position": cache_position,
            }
        )
        return model_inputs
